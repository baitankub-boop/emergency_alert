  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/send_registration_otp", {
        ...
        body: JSON.stringify({ first_name: firstName, last_name: lastName, email, password, type: "superadmin" }),
      });
      ...
      if (!res.ok) {
        ...
        router.push(`/verify_registration_otp?email=${encodeURIComponent(email)}&type=superadmin`);
      }
    } catch {
