...
export default function AddOperatorPage() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      if (session && sessionStorage.getItem("google_register_type") === "operator") {
  ...
  const handleGoogleRegister = async () => {
    ...
    sessionStorage.setItem("google_register_type", "operator");
    ...
    try {
      if (googleVerified) {
        ...
        const res = await fetch("/api/create_staff_google", {
          ...
          body: JSON.stringify({ type: "operator", first_name: firstName, last_name: lastName, email, password }),
        });
        const json = await res.json();
        if (!res.ok) {
          setError(json.error || "Failed to create operator");
        } else {
