...
export default function StatusPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (activeTable === "emergency") {
      const { data, error } = await supabase
        .from("emergency_data")
        .select("id, created_at, emergency_type, floor, description, email, status, remark")
      ...
      const { data, error } = await supabase
        .from("breakdown_data")
        .select("id, created_at, breakdown_type, floor, description, email, status, remark")
        .order("created_at", { ascending: false });
  ...
  const displayEmergency = (myReportsOnly && userEmail
    ...
    : emergencyRows
  ).filter(r => !floorFilterEmergency || r.floor === floorFilterEmergency);

  const displayBreakdown = (myReportsOnly && userEmail
    ? breakdownRows.filter(r => emailMatch(r.email, userEmail))
    : breakdownRows
  ).filter(r => !floorFilterBreakdown || r.floor === floorFilterBreakdown);
  ...
  const emergencyCols = (userEmail ? 7 : 6) + 1;
  const breakdownCols = (userEmail ? 7 : 6) + 1;
      ...
      <div className="bg-gradient-to-br from-slate-50 to-white border-b border-slate-100">
        <div className="container mx-auto px-4 py-10 text-center">
          <div className="inline-flex items-center gap-2 bg-red-50 border border-red-200 rounded-full px-4 py-1.5 mb-4">
            ...
            <span className="text-red-600 text-xs font-semibold tracking-wide uppercase">{t("live_monitoring")}</span>
          </div>
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-900 mb-2 animate-fadeInUp">
            {t("home_title")}
          </h1>
          <p className="text-slate-400 text-sm animate-fadeInUp delay-100">
            40 Building · KMUTNB
          </p>
        </div>
      </div>
